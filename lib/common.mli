...
module Make
  ...
  : sig
    ...
    (** [run flow m] runs [m] on [flow]. [m] is a description of what we will
       send and receive. [flow] is a representation of the {i socket}. *)
    ...
    val recipients_are_reachable
      ...
      -> bool IO.t
    (** [recipients_are_reachable ~ipv4 resolver recipients] tries to resolve {i MX}
       record for any domains of recipients. [ipv4] is the {b public} IPv4 of the
       current SMTP server to resolve [Postmaster]. *)
    ...
    val resolve_recipients
      ...
      (** [resolve_recipients ~domain resolver map recipients] tries to resolve
         [recipients] with [resolver] and rightly translates recipients found
         into [map] to expected recipients.

         The result is a list of:
         {ul
         {- [`Domain (domain, mxs)] which is an aggregation of all recipients
          with the same [domain]. [mxs] is a set of Mail eXchange services
          (priority and IP address) of the [domain].}
         {- [`Ipaddr ip] is an aggregation of all recipients with the same IP
          as a domain.}} *)
  end
