...
module type MONAD = sig
  ...
  val recv
    :  context
    -> 'a Value.recv
    -> ('a, [> `Protocol of error ]) Colombe.State.t

  val return : 'a -> ('a, 'err) Colombe.State.t
  val fail : 'err -> ('a, 'err) Colombe.State.t
  val reword_error : ('err0 -> 'err1) -> ('v, 'err0) Colombe.State.t -> ('v, 'err1) Colombe.State.t
  val error_msgf : ('a, Format.formatter, unit, ('b, [> R.msg ]) Colombe.State.t) format4 -> 'a
end
