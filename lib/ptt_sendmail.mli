(** The purpose of this module is to launch a ‘job’ which will send the emails
    ...
    server information (so that it can identify itself to other SMTP servers), a
    TLS client configuration (to initiate encrypted communication with STARTTLS)
    and a happy-eyeballs instance to initiate a TCP/IP connection with the SMTP
    servers.

    An email that you want to send is described in such a way that it contains
    the sender, the recipients, the content, a unique ID and an error management
    policy.
