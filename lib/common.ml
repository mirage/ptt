...
struct
  ...
  let rdwr =
    ...
    {
      Colombe.Sigs.rd =
        (fun flow buf off len -> inj (Flow.recv flow buf off len));
      Colombe.Sigs.wr =
  ...
  let recipients_are_reachable ~ipv4 w recipients =
    ...
    let rec go acc = function
      ...
      | Forward_path.Forward_path { Path.domain = Domain.IPv4 mx_ipaddr; _ }
        :: r
      ...
      | Forward_path.Domain (Domain.Extension (ldh, v)) :: r -> (
          ...
          | Ok mx_ipaddr ->
              go
                (Mxs.(singleton (v ~preference:0 (Ipaddr.V4 mx_ipaddr))) :: acc)
                r
          ...
          let fold mxs { Dns.Mx.mail_exchange; Dns.Mx.preference } =
            ...
            | Ok mx_ipaddr ->
                let mxs =
                  Mxs.(
                    add
                      (v ~preference ~domain:mail_exchange
                         (Ipaddr.V4 mx_ipaddr))
                      mxs) in
                IO.return mxs
