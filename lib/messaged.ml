...
let v ~domain_from ~from ~recipients id =
  { domain_from
  ; from
  ; recipients
  ; id }
...
= struct
  ...
  let pipe_of_queue ?(chunk= 0x1000) queue =
    if chunk <= 0
    then Fmt.invalid_arg "stream_of_queue: chunk must be bigger than 0" ;
    ...
    let consumer () =
      ...
      let rec wait () =
        ...
        then Condition.wait condition mutex >>= wait
        else return () in
      wait () >>= fun () ->
      let len = min (Ke.length queue) (Bytes.length buf) in

      if len = 0 && !close
      then ( Mutex.unlock mutex ; return None )
      else
        ( Ke.N.keep_exn queue ~blit:blit_to_bytes ~length:Bytes.length ~off:0 ~len buf
    ...
    { q= queue; m= mutex; c= condition; f= close; },
    producer, consumer

  let close queue =
    Mutex.lock queue.m >>= fun () ->
    queue.f := true ; Mutex.unlock queue.m ; return ()
  ...
  let await t =
    ...
    let rec await () =
      ...
      then Condition.wait t.c t.m >>= await
      else return () in
