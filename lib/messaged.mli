...
module type S = sig
  ...
  (** [close queue] notifies [queue] to be closed. Then, {!producer} will
      {i eat} anything an {!consumer} will return as soon as possible [None]. *)

  val push : ?chunk:int -> t -> key -> chunk producer s
  (** [push ?chunk messaged key] adds a new message into the dispatcher and give
      to the caller the {!producer} to fill the message asynchronously. *)

  val await : t -> unit s
  (** [await t] awaits a new message from the dispatcher.

      {b Note.} by design, a server should never stop, [await] can infinitely
      blocks the process as long as there is no message available in [t]. *)

  val pop : t -> (key * queue * chunk consumer) option s
  (** [pop t] returns next message available in [t]. It gives the identifier,
      the queue which contains the mail and a safe-threaded consumer. *)
