...
module Protocol = struct
  ...
  let connect sockaddr =
    ...
    Ok { socket; linger; sockaddr; closed = false }
  ...
  let rec recv ({ socket; closed; _ } as t) raw =
    ...
    then
      ...
      try
        ...
        let len =
          Unix.read socket t.linger 0 (min max (Bytes.length t.linger)) in
        if len = 0
        then Ok `End_of_flow
        else (
          ...
          then
            ...
            then Ok (`Input len)
              ...
              let ( >>= ) x f =
                match x with Ok x -> f x | Error err -> Error err in
  ...
  let rec send ({ socket; closed; _ } as t) raw =
    ...
    then Error Socket_closed
      ...
      try
        let len1 = Unix.write socket t.linger 0 len0 in
        if len1 = len0
        then
          if max > len0
...
type configuration = { inet_addr : Unix.inet_addr; port : int; capacity : int }
